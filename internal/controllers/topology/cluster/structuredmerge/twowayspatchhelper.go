...
func NewTwoWaysPatchHelper(original, modified client.Object, c client.Client, opts ...HelperOption) (*TwoWaysPatchHelper, error) {
	...
	filterObjectInput = filterObjectInput.ApplyOptions(opts)
	filterObjectInput.AllowedPaths = []contract.Path{
		{"metadata", "labels"},
	...
	if util.IsNil(original) {
		filterObjectInput.AllowedPaths = append(filterObjectInput.AllowedPaths,
